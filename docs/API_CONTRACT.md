...
- 판정은 **stateless** 다. 서버는 세션·판정 결과를 **다시 조회할 수 있는 형태로**
  저장하지 않으며, 클라이언트가
...
- 판정 상태·사유 코드는 **비식별 운영 로그**로 남는다 (ADR-0002). 좌표·조건 원문·
  발행시각 원값·장소 이름·세션 식별자는 로그에 포함되지 않으므로, 로그로부터 요청을
  복원할 수 없다.
- JSON 으로 파싱되지만 객체가 아닌 본문(`null`·문자열·숫자)은 `SAFEHOUR_BAD_REQUEST`
  400 이다.
