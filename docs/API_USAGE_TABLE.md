...
| `mapx`·`mapy` | 거리·이동시간 계산 입력 + **동일 장소 판정** | 화면에 좌표를 직접 표시하지 않음. 같은 이름이 100m 안에 있으면 중복 등록으로 보고 합침 |
| 무장애 상세 | 접근성 정보 | 신호 존재 여부만. 보행부담 보정에 **최대 20%** 반영 |
| 기상 `PTY`·`RN1`·`T1H` | "기상 실황" 문구 | 임계값 판정 후 실외 후보 제외 근거로 사용 |

동일 장소가 `contentId` 를 달리해 여러 건 등록된 경우가 있다(예: 코엑스 아쿠아리움
`2507822`·`130284`). 추천은 3건뿐이라 중복 하나가 선택지를 실질 하나 줄이므로,
**이름이 같고 좌표가 100m 이내일 때만** 하나로 합친다. 이름만으로는 합치지 않는다 —
같은 브랜드의 다른 지점을 잃기 때문이다. 영문 원문이 있는 쪽을 남긴다.
...
- 스냅샷은 **보조 증빙**이다. 다만 포털은 이용자에게 키별 호출 통계를 제공하지 않으므로
  (2026-08-25 마이페이지 확인), **우리가 볼 수 있는 유일한 지표**이기도 하다.
