...
| --- | --- | --- | --- |
| 시작·안전 안내 | `/` | 없음 | 없음 |
| 조건 입력 | `/plan` | `POST /api/recommend` | 관광 3종 + 기상 |
...
`docs/decisions/0001-route-contract.md` 참고.

## 4. 데이터가 화면에 쓰이는 방식

| 공공 API 필드 | 화면 표시 | 가공 여부 |
| --- | --- | --- |
| `title` | 추천 카드 제목 | 영문 우선·국문 폴백. **원문 그대로**, 폴백 시 "번역 필요" 배지 |
| `overview` | 장소 상세 관광정보 원문 | **원문 그대로.** SafeHour 문구를 섞지 않음 |
| `addr1`·`addr2` | 장소 상세 주소 | 결합만 |
| `usetime`·`restdate` 등 | 운영·휴무 정보 | 원문 그대로 + 한국어 라벨. **영업 여부는 판단하지 않음** |
...
| 기준점 좌표 로그 | **미기록** | `mapX`/`mapY`/`contentId` 를 `REDACTED` 로 가림. 호출 로그와 캐시 파일 **둘 다** (D07-POL002, 테스트로 고정) |

이 절은 **TourAPI 호출 로그**에 대한 것이다. 판정 결과 로그는 성격이 달라
`docs/DEPLOYMENT.md` 1.3 과 ADR-0002 에서 따로 다룬다.

경계 동작은 `test/tour-api-limits.test.js` 25건으로 고정돼 있다 —
799/800/999/1000/1001, 병렬 호출 시 한도 초과 방지, 캐시 적중 시 미증가,
좌표·contentId redaction(로그와 캐시 양쪽), 만료 캐시 정리,
로그 쓰기 실패 시 호출 결과 보존.
...
| 위치 | 문구 |
| --- | --- |
| 결과 화면 하단 | "관광정보 출처: 한국관광공사 TourAPI (국문·영문·무장애). 이동시간은 경로 API 연결 전까지 보수적 추정값입니다. SafeHour는 의료진의 판단을 대체하지 않습니다." |
| 장소 상세 원문 아래 | "출처: 한국관광공사 TourAPI (TourAPI:KorService2 등 실제 사용 서비스)" |
