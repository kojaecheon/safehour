...
| AX-003 | D03 라우트·화면 계약 결정 | SCR002–SCR006, F013 | 합쳐진 `/plan` 승인 또는 단계 라우트 분리 ADR | ADR 작성 완료 (`docs/decisions/0001-route-contract.md`) · 사람 승인 대기 |
| AX-010 | 라우트 결정 보완 조건 구현 | ADR-0001, D03-NAV004 | 입력 유지(draft), 모달 뒤로가기, `/place` 차단 가드, 변화 요약 잔존, `/plan` 앵커 | 대기 (ADR 승인 후) |
...
| AX-006 | API 한도·부분 실패 경계 테스트 | AC017, QA032 | 800 경고, 1,000 차단, 상세 일부 실패 회귀 | 대기 |
| AX-009 | 안전 핵심 모듈 커버리지 보강 | D09 QA, D04-F003·F010 | client·candidate-service 오류/캐시 경계, 전체 line 80% 목표 재평가 | 대기 |
| AX-007 | 기상 실API 증거 갱신 | F011, AC011 | 승인된 키로 실응답, 실패·stale 증거 | 외부 의존 |
| AX-008 | 개인정보 안전 계측 설계 | EVT001–EVT007, POL008 | schema·동의·보존 승인, PII 0 fixture | 결정 필요 |
...
| AX-102 | 접근성·반응형 검증 | AC019, QA033–036 | axe, 키보드, 200%, 스크린리더, 실기기 | 대기 |
...
| ID | 작업 | 연결 | 완료 조건 | 상태 |
| --- | --- | --- | --- | --- |
| AX-201 | 계측 dashboard와 운영 alert | D02 이벤트, RG007 | 퍼널·변화율·API 성공률·누락 경고 | 대기 |
| AX-202 | 운영계정·API 활용표 | RG004, RG008 | 배포 URL, 호출 증적, 화면 대응표 | 대기 |
