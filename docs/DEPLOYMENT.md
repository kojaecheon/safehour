...
헬스·**로그인 가능 여부**·보안 헤더·안전 게이트 3종·판정과 재판정·비밀정보 노출을 확인한다.

> **로그인 검사를 넣은 이유.** 배포본에 로그인 수단이 하나도 없어 지침 연결부터
> 막혔는데 검증 13건이 전부 통과한 적이 있다. 제품은 그때도 `/api/auth/session` 으로
> 정확히 보고하고 있었다 — 사람이 눈으로 볼 때만 보였고 자동 검증이 보지 않았다.
>
> 판정은 `auth.ready` 만으로 하지 않는다. 그것은 "세션 키와 콜백 주소가 있다" 는
> 뜻이고 로그인 **수단**이 있는지는 보지 않아, `ready=true` 인데 아무도 로그인할 수
> 없는 조합이 존재한다. 검증은 **기반 + 수단 하나 이상**을 함께 본다.
