...
엔진·TourAPI·모바일 웹·E2E·배포 준비는 실행 가능한 수준에 도달했다. 자동 테스트 336건과
...
**배포는 해소됐다 (2026-08-18 머지, 08-25 환경변수 반영).** `safehour.vercel.app` 에서
로그인 → 병원 지침 연결 → 외출 판정까지 동작하며, 한국관광공사 대외개방운영팀이
이 배포본으로 국문 관광정보 활용을 확인하고 운영계정을 승인했다.
남은 차단은 **Google·Kakao OAuth 앱 등록**으로, 현재는 데모 경로로만 진입할 수 있다.
...
| 단위·통합 테스트 | `npm test` | **336/336 통과** (82 suites) |
...
| 공공 API 호출 이력 | `npm run usage:weekly` → `docs/API_USAGE_SNAPSHOT.md`. 현재 **108콜/5일** | **축적 필요** — 08.20~09.01 공백, 소급 불가 |
| 배포 | https://safehour.vercel.app — 최신 반영, `ready: true`·`demoLogin: true` | 준비 |
| 소셜 로그인 자격증명 | 코드·테스트 완료. Google·Kakao 앱 등록 미완 — 데모 경로로 대체 중 | **운영자 실행 필요** |
...
| Dxx 기록 | 실제 | 조치 |
| --- | --- | --- |
| D09: 자동 테스트 46개 | **336개** + E2E 93개 | D09 갱신 필요 |
