...
엔진·TourAPI·모바일 웹·E2E·배포 준비는 실행 가능한 수준에 도달했다. 자동 테스트 271건과
E2E 91건이 CI에서 통과하고, 추천을 즉시 중단할 kill switch와 롤백 절차가 있다.
...
| 단위·통합 테스트 | `npm test` | **271/271 통과** (66 suites) |
| 테스트 커버리지 | `npm run test:coverage` | **line 95.60%, branch 84.13%, function 94.12%** |
| E2E | `npm run e2e` (Playwright, 360px) | **91/91 통과** (6 파일), 외부 API 호출 0건 |
...
| Dxx 기록 | 실제 | 조치 |
| --- | --- | --- |
| D09: 자동 테스트 46개 | **271개** + E2E 91개 | D09 갱신 필요 |
...
| EVT001–EVT007 계측 | 미구현 | **ADR-0002 결정 후** 구현 (선택지 3개·권고 B 정리 완료) |
...
- 계측 이벤트 (동의 정책 승인 후 — ADR-0002 결정 대기)
- 운영 대시보드·알림 (AX-201)
