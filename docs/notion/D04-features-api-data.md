...
- 자동 테스트: 단위·통합 **322개 통과 · 77 suites** (`npm test`, 2026-09-03 실측) + **E2E 93건 · 6파일** (`npx playwright test --list`)
...
### D04-F027 휴무 후보 제외 — **신규 · 부분 구현**

- 가치: 문을 닫은 곳으로 회복기 환자를 보내지 않는다. 헛걸음은 복귀 마감을 위협한다.
- 연결: D01-PRB002, D02-S005, D05-AUTO001
- 구현: `src/tour-api/schedule.js` · `src/tour-api/candidate-service.js`
- **읽는 것은 둘뿐이다.**
  1. 행사 기간 (`eventstartdate`·`eventenddate`) — `YYYYMMDD` 구조화 필드라 해석 여지가 없다
  2. 정기 휴무 요일 (`restdate` 계열의 "매주 월요일 휴무") — **오늘 요일과 정확히 맞을 때만**
- **금지: "지금 영업 중" 을 단정하지 않는다.** TourAPI 운영시간은 자유 텍스트다.
  실데이터에는 `"10:30~22:00<br>※ 일부 영업시간이 다른 매장이 있음"`,
  `"N/A (Open all year around)"` 같은 문장이 섞여 있다. 여기서 영업 여부를 추정하면
  D07-BAN007(불확실성을 사실로 표시 금지) 위반이다.
- 근거가 명백하지 않으면 `openNow` 를 `null` 로 둔다. 이것은 "열려 있다" 가 **아니라**
  "닫혔다는 근거가 없다" 는 뜻이다.
- `연중무휴`·`무휴`·`N/A`·`open all year` 가 있으면 요일 탐색을 하지 않는다.
- 조회 실패는 닫힘이 아니다. 실패하면 그 후보의 `openNow` 는 `null` 로 남는다.
- 상태: **부분 구현** — 위 두 근거만 판정한다. 임시 휴업·행사로 인한 변경·실시간
  혼잡은 감지하지 못한다. 교통 지연도 여전히 공급자가 없다.
- 우선순위: P0
...
- 용량: operation별 1,000회/일 자체 차단. **국문 관광정보는 운영계정 승인**(2026-08-25, 한국관광공사 대외개방운영팀 실사) · 나머지 3종은 개발계정. 활용신청 4건 전부 승인, 2028년까지 유효. 로컬 누적 9콜(2일) · 실제 증적은 포털의 키별 집계.
...
| U-04 | **금식 종료 시각이 boolean 으로 접힌다** | `fastingUntil`(`HH:MM`)이 "식음 제한이 있거나 종료시각이 있으면 금식" 이라는 참·거짓 하나로만 쓰인다. "13:00 이후 식음 가능" 이 표현되지 않는다 |
