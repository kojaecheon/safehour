...
/** 허용 재계산 트리거 (D02-EVT004 의 trigger_type). 판정 로그도 이 집합을 쓴다. */
...
/**
 * 후보의 수치 필드를 판정에 쓸 수 있는 값으로만 좁힌다.
 *
 * 재계산 payload 는 클라이언트가 통째로 되돌려 보내므로 후보 안의 값도
 * 신뢰할 수 없다. JSON 의 `1e999` 는 JS 에서 Infinity 로 파싱되고, 그 값이
 * 체류시간으로 들어가면 shrinkToFit 의 `stay -= 5` 가 영원히 줄지 않아
 * 이벤트 루프가 멈춘다 — 예외가 아니라 멈춤이라 라우트의 try/catch 도
 * 판정 로그도 작동하지 않는다. 여기서 막는 것이 가장 바깥이다.
 *
 * 범위를 벗어난 값은 요청을 거부하지 않고 버린다. 거부하면 환자 호출·
 * 위험신호 재판정까지 막히는데, 그쪽이 더 위험하다 (D04-BR011).
 * 값이 없으면 엔진이 보수적 기본값을 쓴다.
 */
const CANDIDATE_NUMERIC_LIMITS = {
  stayMin: 24 * 60,
  walkMin: 24 * 60,
  travelMin: 24 * 60,
  lat: 90,
...
function sanitizeCandidate(candidate) {
  ...
  for (const [key, max] of Object.entries(CANDIDATE_NUMERIC_LIMITS)) {
