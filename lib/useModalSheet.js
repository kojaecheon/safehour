...
export function useModalSheet(onClose) {
  ...
  useEffect(() => {
    ...
    // NOTE: 뒤로가기로 시트만 닫는 처리(ADR-0001 보완 조건 3)는 여기 넣지 않는다.
    // pushState + cleanup 의 history.back() 왕복이 StrictMode 이중 마운트와 충돌해
    // 시트가 열리자마자 닫혔다. 안전 시트를 불안정하게 만들 수 없어 AX-011 로 분리한다.
