 ...
 * 공모전 1차심사 제출항목 4(공공데이터 활용 실적)의 증빙이다.
 *
 * 포털은 **이용자에게 키별 호출 통계를 보여주지 않는다** (2026-08-25 마이페이지 확인 —
 * 하위 메뉴는 활용신청 현황·인증키 발급현황·오류신고현황뿐이고, 상세 페이지에도
 * 일일 트래픽 한도만 있다). 심사 측이 기관을 통해 조회하므로 우리는 미리 볼 수 없다.
 * 따라서 이 표가 호출이 실제로 나가는지 확인할 수 있는 유일한 수단이다.
 *
